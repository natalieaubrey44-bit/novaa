...
const ADMIN_NAV = [
  ...
  { id: 'accounts', label: 'Accounts', href: '/admin/accounts', icon: CreditCard },
  { id: 'transactions', label: 'Transactions', href: '/admin/transactions', icon: FileText },
        ...
        <aside className="hidden lg:flex lg:flex-col lg:w-80 lg:fixed lg:top-0 lg:left-0 lg:h-screen gap-8 border-r border-white/10 bg-brand-navy px-6 py-8 overflow-visible">
          <div className="space-y-4">
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-4">
              ...
              <p className="mt-3 text-base font-semibold text-white">{user?.name || 'Administrator'}</p>
          ...
          <button
            type="button"
            onClick={logout}
            className="mt-auto inline-flex items-center justify-center gap-2 rounded-3xl bg-white/5 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-white/10"
          >
            <LogOut size={16} />
            Sign out
          </button>
          ...
          <header className="border-b border-white/10 bg-slate-950/90 px-6 py-5 sm:px-8">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <h2 className="text-3xl font-semibold tracking-tight text-white">Control center</h2>
              </div>
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-400">
                ...
                <span className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-2 text-slate-300">{new Date().toLocaleDateString()}</span>
              </div>
            </div>
          </header>
